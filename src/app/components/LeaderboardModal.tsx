...
import { 
  ...
  ChevronLeft, Sun, ClipboardList, Building2
} from "lucide-react";
  ...
  const content = (
    <div className={`flex flex-col ${isPage ? "gap-4 w-full" : "w-full max-h-[90vh] overflow-hidden"}`}>
        ...
        <div className="flex items-end justify-center gap-3 pt-2">
          ...
          {top3[1] && (
            <div className="flex-1 flex flex-col items-center text-center">
              <div className="w-12 h-12 rounded-2xl bg-slate-100 border-2 border-slate-300 flex items-center justify-center font-bold text-slate-700 shadow-xs relative mb-1.5">
          ...
          {top3[2] && (
            <div className="flex-1 flex flex-col items-center text-center">
              <div className="w-12 h-12 rounded-2xl bg-amber-50 border-2 border-amber-300 flex items-center justify-center font-bold text-amber-800 shadow-xs relative mb-1.5">
      ...
      <div className="space-y-2 pb-6">
        <h4 className="text-xs font-bold text-slate-700 px-1">Daftar Peringkat Musyrif</h4>
        {rest.map((m, idx) => (
          <div
            key={m.id}
            onClick={() => onSelectMusyrif?.(m.id)}
            className="p-3 bg-white border border-slate-200/70 rounded-2xl flex items-center justify-between gap-3 shadow-xs hover:border-emerald-300 cursor-pointer transition-all active:scale-[0.99]"
          >
            <div className="flex items-center gap-3 min-w-0">
              <span className="w-6 text-xs font-bold text-slate-400 font-mono text-center">
                #{idx + 4}
              </span>
