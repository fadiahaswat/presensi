...
export function JurnalLogbookModal({
  ...
  const content = (
    <div className={`flex flex-col ${isPage ? "gap-4 w-full" : "w-full max-h-[90vh] overflow-hidden"}`}>
      {/* Header Bar */}
      <div className={`p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3.5 ${isPage ? "bg-white rounded-3xl border border-slate-100 shadow-sm ring-1 ring-slate-200/60" : "bg-slate-900 text-white rounded-t-3xl sm:rounded-t-[28px]"}`}>
        <div className="flex items-center justify-between gap-3 min-w-0">
          <div className="flex items-center gap-3 min-w-0">
            <button type="button" onClick={onClose} className={`w-10 h-10 rounded-2xl flex items-center justify-center transition-all active:scale-95 shadow-2xs shrink-0 ${isPage ? "bg-slate-50 border border-slate-200/80 hover:bg-slate-100 text-slate-700" : "bg-white/10 hover:bg-white/20 text-white"}`}>
              {isPage ? <ChevronLeft className="w-5 h-5" /> : <X className="w-4 h-4" />}
            </button>
            <div className="min-w-0">
              <h2 className={`font-black text-base sm:text-lg leading-tight truncate ${isPage ? "text-slate-900" : "text-white"}`}>Jurnal Tugas Logbook Musyrif</h2>
              <p className={`text-xs mt-0.5 truncate ${isPage ? "text-slate-400" : "text-slate-300"}`}>Monitoring kedisiplinan dan checklist tugas harian asrama</p>
            </div>
          </div>

          {isMusyrifUser && isGpsVerified && (
            <button type="button" onClick={handleSave} className="sm:hidden px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl text-xs font-bold shadow-xs flex items-center gap-1.5 active:scale-95 transition-all shrink-0">
              <Check className="w-4 h-4" /> <span>Simpan</span>
            </button>
          )}
        </div>
        ...
        <div className="flex items-center gap-2.5 sm:shrink-0 w-full sm:w-auto">
          ...
          {onOpenMutabaah && isPage && (
            <div className="grid grid-cols-2 bg-slate-100/90 p-1 rounded-2xl border border-slate-200/70 shadow-inner w-full sm:w-60">
              <button
                type="button"
                className="px-3.5 py-2 rounded-xl text-xs font-bold transition-all duration-150 flex items-center justify-center gap-1.5 bg-indigo-600 text-white shadow-sm shadow-indigo-600/25 scale-[1.01]"
              >
              ...
              <button
                ...
                className="px-3.5 py-2 rounded-xl text-xs font-bold transition-all duration-150 flex items-center justify-center gap-1.5 text-slate-600 hover:text-rose-600 hover:bg-white/40"
              >
          ...
          {isMusyrifUser && isGpsVerified && (
            <button type="button" onClick={handleSave} className="hidden sm:flex px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl text-xs font-bold shadow-xs items-center gap-1.5 active:scale-95 transition-all shrink-0">
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="truncate">
                      {selectedMusyrif
                        ? (selectedMusyrif.id === mySelfMusyrif?.id
                            ? `${selectedMusyrif.name} (Saya Sendiri)`
                            ...
                            >
                              <div className="min-w-0">
                                <div className="truncate">
                                  {m.id === mySelfMusyrif?.id ? `${m.name} (Saya Sendiri)` : m.name}
                                </div>
