  ...
  return (
    <div className="space-y-3.5 max-w-7xl mx-auto pb-10">
      ...
      <div className="bg-white rounded-3xl p-4 sm:p-5 shadow-sm ring-1 ring-slate-200/70 border border-slate-100/50 flex flex-col gap-3.5">
        {/* Top Header Card */}
        <div className="flex items-center justify-between gap-2.5 pb-1 border-b border-slate-100">
          <div className="flex items-center gap-2 min-w-0">
            <button
              type="button"
              onClick={onBack}
              className="w-7 h-7 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 shadow-2xs flex items-center justify-center transition-all shrink-0 active:scale-95"
              title="Kembali ke Dasbor"
            >
              <ChevronLeft className="w-3.5 h-3.5" />
            </button>
            <div className="w-7 h-7 rounded-lg flex items-center justify-center shadow-xs flex-shrink-0 bg-emerald-600 text-white shadow-emerald-600/20">
              <CalendarIcon className="w-3.5 h-3.5"/>
            </div>
            <div className="min-w-0">
              <div className="flex items-center gap-1.5 flex-wrap">
                <span className="inline-flex items-center gap-1 px-1.5 py-0.2 rounded-full bg-emerald-50 text-emerald-800 border border-emerald-200/80 text-[9px] font-bold font-mono">
                  <span className="w-1 h-1 rounded-full bg-emerald-600 animate-pulse" />
                  KHGT Majelis Tarjih
                </span>
                <span className="text-[9px] text-slate-400 font-mono hidden xs:inline">PP Muhammadiyah</span>
              </div>
          ...
          <button
            ...
            className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-[11px] font-bold ring-1 transition-all shadow-2xs shrink-0 active:scale-95 ${
              showKHGTInfo
        ...
        {showKHGTInfo && (
          <div className="p-3 rounded-xl bg-emerald-50/60 border border-emerald-200/80 text-emerald-950 text-xs space-y-2">
            <div className="flex items-center justify-between">
              <p className="font-extrabold text-xs text-emerald-900 flex items-center gap-1.5">
                <BookOpen className="w-3.5 h-3.5 text-emerald-700" />
                Prinsip KHGT Muhammadiyah
              </p>
              <button
                type="button"
            ...
            <p className="leading-relaxed text-slate-700 text-[11px]">
              Berdasarkan keputusan <strong>Munas Tarjih ke-32</strong> & <strong>Keputusan PP Muhammadiyah No. 120/KEP/I.0/B/2024</strong>, KHGT resmi berlaku mulai 1 Muharram 1446 H.
            </p>
          </div>
        )}

        {/* Integrated Hijri Month Navigator - Compact */}
        <div className="flex items-center justify-between bg-slate-50/80 rounded-xl p-1 border border-slate-100/80">
          <button
            type="button"
            onClick={handlePrevMonth}
            className="w-7 h-7 rounded-lg bg-white shadow-2xs hover:bg-slate-100 text-slate-600 flex items-center justify-center active:scale-95 transition-all shrink-0"
            title="Bulan Sebelumnya"
          >
            <ChevronLeft className="w-3.5 h-3.5" />
          </button>
          
          <div className="text-center px-1.5 min-w-0">
            <p className="text-xs sm:text-sm font-extrabold text-slate-800 font-mono leading-tight truncate">
              {HIJRI_MONTHS[selectedMonth - 1]} {selectedYear} H
            </p>
            {daysInMonth.length > 0 && (
              <p className="text-[9px] text-slate-400 font-mono mt-0.5 truncate leading-tight">
          ...
          <div className="flex items-center gap-1 shrink-0">
            <button
              ...
              className="px-2 py-1 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-[10px] active:scale-95 shadow-2xs transition-all font-mono"
            >
              Hari Ini
            </button>
            <button
