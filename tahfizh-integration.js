...
async function loadTahfizhHTMLStructure() {
    try {
        const response = await fetch('/tmp/workspace/fadiahaswat/presensi/sistemtahfizh-main/index.html');
        ...
        console.warn('Could not load tahfizh HTML from file, using fallback:', error);
...
async function loadTahfizhScripts() {
    try {
        // First load config
        if (!window.AppConfig) {
            const configResponse = await fetch('/tmp/workspace/fadiahaswat/presensi/sistemtahfizh-main/config.js');
